...
class EnhancedInstagramAutomator:
    ...
    def check_session_validity(self) -> bool:
        ...
        if not self.page:
            self._debug_log("Session check skipped - page not initialized")
            return True  # Assume valid if page not set up yet
    ...
    def robust_upload_video(self, file_path: str, caption: str = "", max_retries: int = None) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Check session validity (only if page is set up)
                if self.page and not self.check_session_validity():
    ...
    def login(self, username=None, password=None, headless=True, timeout=45000):
        ...
        For EnhancedInstagramAutomator, context should be set up before calling this.
        """
        self.last_error = None
        ...
        try:
            ...
            # Check for Facebook redirect
            if "facebook.com" in self.page.url:
                self._debug_log(f"Detected Facebook redirect. Forcing return to Instagram...")
                self.page.goto("https://www.instagram.com/", wait_until="domcontentloaded", timeout=timeout)
                self.page.wait_for_timeout(3000)
            ...
            self._debug_log("Session invalid - redirected to login page")
            return False
